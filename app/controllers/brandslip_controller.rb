...
class BrandslipController < ApplicationController
  ...
  def search_suggestion_filter
    ...
    if(params[:category_arr].include? "-1")
      @suggestions = BrandslipSuggestion.find_by_sql("select * from brandslip_suggestions where is_assigned is null and DATE_ADD(created_at, INTERVAL valid_for day)>now() and user_id not in (#{params[:suggestion_user_id]}) #{crowd_size_clause} #{money_clause} #{sub_category_clause} #{suggestion_type} #{online_media_source_clause} #{follow_sub_clause} order by id desc")
    else
      category_str = params[:category_arr].join(",")
      @suggestions = Job.find_by_sql("select * from brandslip_suggestions where is_assigned is null and DATE_ADD(created_at, INTERVAL valid_for day)>now() and user_id not in (#{params[:suggestion_user_id]}) #{crowd_size_clause} #{money_clause} and category in (#{category_str}) #{sub_category_clause} #{suggestion_type} #{online_media_source_clause} #{follow_sub_clause} order by id desc")
    end
       ...
       if(!job[0]['assigned_to'].nil?)
         ...
         if(!job_proposal_details.nil?)
          ...
          begin
            ...
            amount_in_cents = (proposal_cost.to_i * 100).to_s
